...
After calculating a discriminator's predictions over the test genome, we can generate the Manhattan plot. The program takes arguments of the discriminator's prediction (generated by `genome_disc.py`) and the outpath for the manhattan plot.
An example command:
~~~
python3 plot_manhattan.py output/correlation/prob_train_pop_A_test_pop_B_disc1.txt manhattan_plots/pop_A/mh_train_pop_A_test_pop_B_disc1.pdf
~~~
...
To calculate population summary statistics, the `genome_stats.py` program takes the path to your test population's h5 file, the path to your mask (bed) file, and the output path as arguments. The summary statistics are calculated for each region found, and saved to a numpy array.

Suppose population B is our test population, and that population B uses the mask `strict_mask.bed`. We would use the command:
~~~
python3 genome_stats.py pop_B.h5 strict_mask.bed output/prediction/pop_B_summary_stats.npy
~~~
...
~~~
python3 correlation_heatmap.py output/correlation/pop_B_summary_stats.npy output/correlation/hidden_train_pop_A_test_pop_B_disc1.npy figures/discriminator/heatmaps/train_pop_A_test_pop_B_disc1.pdf "disc1, train: pop A, test: pop B"
~~~
